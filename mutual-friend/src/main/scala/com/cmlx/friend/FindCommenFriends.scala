...
object FindCommenFriends {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //1、获取原始的好友数据
    val records: RDD[String] = sc.textFile("mutual-friend/src/main/resources/friend/friends.txt")
    ...
    val sql = "SELECT t1.targetId FROM alita_app_user_follow t1" +
      " JOIN alita_app_user_follow t2 ON t1.targetId = t2.uid " +
      " JOIN alita_app_user t3 ON t1.targetId = t3.uid" +
      " WHERE t1.uid = " + uid +
      " AND t2.targetId = " + uid +
      " AND t1.dataState = 2" +
      " AND t2.dataState = 2" +
    ...
    val rdd = sparkSession.sql(sql).toString()
